...
<html lang="zh">
  <head>
    ...
    <script>
    async function runcode(code,cin,type,stat){
        console.log(code,type);
        if(type == "c_cpp"){
          var res=await fetch("<?php echo $ApiUrl?>?code="+encodeURIComponent(code)+"&cin="+encodeURIComponent(cin)+"&stat="+stat);
          var data = await res.text();
          if(stat=="load"){
            console.log( data);editor.setValue(data);
	  }
	  else $("#cout").html("<pre class=\"fillall\">" + data.replace(/</g,"<").replace(/>/g,">").replace(/\n/g,"<br>") + "</pre>");
        }
    ...
    <style type="text/css" media="screen">
      ...
      .title{height: 5px;line-height: 0;}
      #editor {margin: 0;height:100%;position: absolute;top: 5%;bottom: 0;left: 0;right: 0;}
      .inout{margin: 0;height:100%;width:100% ;top: 15px;}
      ...
      .editor {height: 75%;margin: 0;position: absolute;top: 15px;bottom: 0;left:0;right:0%;}
      .bts {margin: 0;position: absolute;top: 80%;bottom: 0%;left: 0%;right: 0%;}
      .col-md-2 {margin: 0;position: absolute;top: 0;bottom: 0;left: 83.333%;right:0;}
      .form-control {width: auto;}
      .command-group {margin: 0;height:50%;width:100% ;top: 0%;bottom: 0;left: 0;right: 0;} 
    </style>
  </head>
  <body>
    <div>
      <div>
	<h3 class="title">SimpleWebIDE - 
	  ...
	  <button class="btn btn-default" id="cloudsave">云端保存</button>
          <button class="btn btn-default" id="cloudload">云端读取</button>
    ...
    <script>
      ...
      $("#cheak").click(function(){
        runcode(editor.getValue(),cin.getValue(), $("#language").val(),"run");
      });
      ...
      $("#cloudsave").click(function(){
        runcode(editor.getValue(),cin.getValue(), $("#language").val(),"save");
      });
      $("#cloudload").click(function(){
        var result = runcode(editor.getValue(),cin.getValue(), $("#language").val(),"load");
      });
    </script>
  </body>
</html>
